...
void chat(int client_or_server,int server_port,int broad_caster){
    ...
    if (client_or_server == 2) {
        ...
        printf("your chat socket :%d",server_socket);
        struct sockaddr_in server_address;
        server_address.sin_family = AF_INET;
        server_address.sin_port = htons(server_port);
        server_address.sin_addr.s_addr = INADDR_ANY;
     ...
     for(;;) {
        ...
        for(i = 0; i <= fdmax; i++) {
            if (FD_ISSET(i, &read_fds)) { // we got one!!
                if (i == server_socket && (client_or_server == 2)) {
                    ...
                    if (newfd == -1) {
                        ...
                        printf("selectserver: new connection from chat on socket %d\n",newfd);
                ...
                if(i == STDIN_FILENO){
                    ...
                    printf("buf 0 is %c",buf[0]);
                        ...
                        if (!strcmp(tmp[0],"1") || !strcmp(tmp[0],"2")) {
                            ...
                            chat(atoi(tmp[0]),atoi(tmp[1]),udp_socket);
                        }
